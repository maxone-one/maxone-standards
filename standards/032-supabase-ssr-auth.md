# 032 — Supabase SSR Auth: Middleware-Matcher umfasst alle Routes
...
→ Audit reproduzierbar via `node scripts/audit.mjs --standard=032 --local-only`.
...
- Falls ja: enthält das `config.matcher` einen Negative-Lookahead-Pattern
  ...
  - Selektive Liste mit weniger als ~10 Pfaden → WARN (möglicher Bug 032)
